new Vue({
    ...
    methods: {
        ...
        saveMeta() {
            // POST Meta Title
            axios.post('/api/save-page/title', {
            ...
            if(this.checked == true) {
                axios.post('/api/redirect', {
                    path: '\/pages\/' + this.default_url.replace(/ /g, '-').replace(/[^\w-]+/g,''),
                    target: '\/pages\/' + this.url.replace(/ /g, '-').replace(/[^\w-]+/g,'')
                }).then((response) => {
        ...
        default_data: function() {
            return axios.get('/api/meta-tags-page', {
                params: {
                    page_id: $('input[type=hidden]').val()
                }
            }).then((response) => {
                ...
                if(this.title == '' || this.description == '' || this.url == '') {
                    return axios.get('/api/page', {
                        ...
                        var responseData = response.data.body.page;
                        if(this.url == '') {
