new Vue({
    ...
    mounted: function () {
        this.default_shop_name().then(this.get_blog_info).then(this.getWidth).then(this.default_data).then(this.inputCheck).then(() => this.preloading = false);
    },
    methods: {
        // Loading
        loading() {
            return this.show_loading = true;
        },

        // Done
        done() {
            return this.show_loading = false;
        },
        ...
        saveMeta() {
            ...
            // POST Meta Title
            axios.post('/api/save-post/title', {
            ...
            if(this.has_custom_url()) {
                axios.post('/api/redirect', {
                    path: '\/blogs\/' + this.blog_handle + '\/' + this.default_url.replace(/ /g, '-').replace(/[^\w-]+/g,''),
                    target: '\/blogs\/' + this.blog_handle + '\/' + this.url.replace(/ /g, '-').replace(/[^\w-]+/g,'')
                }).then((response) => {
        ...
        get_blog_info: function() {
            return axios.get('/api/blog', {
                params: {
                    post_id: $('input[type=hidden]').val()
                }
            }).then((response) => {
                var responseData = response.data.blog_info;
                this.blog_handle = responseData.blog_handle;
                this.blog_id = responseData.blog_id;
            })
        },
        ...
        default_data: function() {
            ...
            return axios.get('/api/meta-tags-post', {
                params: {
                    post_id: $('input[type=hidden]').val()
                }
            }).then((response) => {
                ...
                if(this.title == '' || this.description == '' || this.url == '') {
                    return axios.get('/api/post', {
                        ...
                        var responseData = response.data.body.article;
                        if(this.url == '') {
                ...
                if(this.keyword && this.keyword.length) {
                    if(this.title.toLowerCase().search(this.keyword.toLowerCase()) >= 0) {
                        ...
                        if(keyword_position == 0) {
                            ...
                            this.messages["good"]["tkp"] = "Your keyword have good position";
                        } else {
                            if(this.messages["good"].hasOwnProperty("tkp")) {
                                delete this.messages["good"]["tkp"];
                            }
                            this.messages["improvement"]["tkp"] = "You shold put important keyword first, but not have to.";
                        }
                    } else {
                        if(this.messages["good"].hasOwnProperty("tkp")) {
                            delete this.messages["good"]["tkp"];
